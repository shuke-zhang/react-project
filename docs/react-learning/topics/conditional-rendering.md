...
根据状态选择渲染、隐藏或替换 React 节点。

核心语法：`return loading ? <Skeleton /> : <Content />`。典型场景：展示加载、空数据、错误、权限和功能开关状态。
...
对应 v-if / v-else；React 使用 if、三元表达式和逻辑与。
...
- 使用 count && <Badge /> 时意外渲染数字 0。
- 为了消除 lint 提示而隐藏依赖或使用 `any`。
- 在 JSX 中堆积复杂表达式，导致业务规则无法测试和复用。

## 推荐写法

- 为复杂状态使用判别联合，确保每个状态都有明确 UI 分支。
...
type RequestStatus = 'loading' | 'success' | 'empty'

/** 演示如何把互斥状态转换为清晰的条件渲染分支。 */
...
真实项目需要补充错误恢复、加载反馈、空状态、无障碍名称和公开行为测试。为复杂状态使用判别联合，确保每个状态都有明确 UI 分支。 对于服务器状态优先使用 TanStack Query，对外部未知数据先通过 Zod 校验，避免一个组件同时承担请求、缓存、校验和展示职责。
