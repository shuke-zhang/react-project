...
在不增加额外 DOM 节点的情况下组合多个子节点。

核心语法：`<Fragment key={item.id}>...</Fragment>`。典型场景：返回多个兄弟节点或为列表片段提供 key。
...
Vue 3 组件原生支持多根节点；React 使用 Fragment 表达。
...
- 为了布局滥用无语义 div，或给短 Fragment 写 key。
- 为了消除 lint 提示而隐藏依赖或使用 `any`。
- 在 JSX 中堆积复杂表达式，导致业务规则无法测试和复用。

## 推荐写法

- 列表片段需要 key 时使用显式 Fragment，而不是短语法。
...
const users = [{ id: 'u1', name: '林晓', role: '开发者' }]
export function Demo() { return <dl>{users.map(user => <Fragment key={user.id}><dt>{user.name}</dt><dd>{user.role}</dd></Fragment>)}</dl> }
```

## 企业级扩展

真实项目需要补充错误恢复、加载反馈、空状态、无障碍名称和公开行为测试。列表片段需要 key 时使用显式 Fragment，而不是短语法。 对于服务器状态优先使用 TanStack Query，对外部未知数据先通过 Zod 校验，避免一个组件同时承担请求、缓存、校验和展示职责。
