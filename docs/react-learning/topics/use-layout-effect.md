# useLayoutEffect 深度教程

## API 概述

在浏览器绘制前读取布局并同步修正 DOM。

核心语法：`useLayoutEffect(setup, dependencies)`。典型场景：测量浮层位置、避免首次绘制闪动。
...
更接近 onMounted 后配合 nextTick 的同步布局测量。
...
- 把普通请求和订阅放入 useLayoutEffect，阻塞浏览器绘制。
- 为了消除 lint 提示而隐藏依赖或使用 `any`。
- 在 JSX 中堆积复杂表达式，导致业务规则无法测试和复用。

## 推荐写法

- 把测量与写入限制在最小范围，并优先尝试 CSS 布局。
...
export function Demo() {
  ...
  useLayoutEffect(() => setWidth(boxRef.current?.getBoundingClientRect().width ?? 0), [])
  return <div ref={boxRef}>绘制前测得宽度：{Math.round(width)}px</div>
}
```

## 企业级扩展

真实项目需要补充错误恢复、加载反馈、空状态、无障碍名称和公开行为测试。把测量与写入限制在最小范围，并优先尝试 CSS 布局。 对于服务器状态优先使用 TanStack Query，对外部未知数据先通过 Zod 校验，避免一个组件同时承担请求、缓存、校验和展示职责。
