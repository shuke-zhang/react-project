...
生成跨服务端与客户端稳定的无障碍关联 ID。

核心语法：`const id = useId()`。典型场景：关联 label/input、帮助文本和错误提示。
...
export function Demo() { const id = useId(); return <><label htmlFor={id}>邮箱</label><input id={id} type="email" /></> }
```

## 企业级扩展

真实项目需要补充错误恢复、加载反馈、空状态、无障碍名称和公开行为测试。用同一前缀派生一组字段 ID，不要用于列表 key。 对于服务器状态优先使用 TanStack Query，对外部未知数据先通过 Zod 校验，避免一个组件同时承担请求、缓存、校验和展示职责。
